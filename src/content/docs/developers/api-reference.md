...
Sync is the encrypted Supabase-backed pipeline described in [Sync & Privacy](/core/sync-privacy/). The frontend drives it through `src/core/sync/` (SyncEngine, ManifestManager, KeyManager, OfflineQueue); the Rust side provides the file primitives and storage access used by that pipeline. Earlier git-based and Iroh P2P sync prototypes were removed — there are no `git_*` or `iroh_*` commands in the current backend.
