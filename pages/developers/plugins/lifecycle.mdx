...
description: Understanding plugin lifecycle states, activation events, event hooks, cleanup procedures, and error handling for robust Lokus plugin development.
keywords: [lokus, plugin lifecycle, activation hooks, event hooks, plugin states, cleanup, error handling, plugin development]
---
...
2. **Cleanup All Resources** - Dispose terminals, tree views, status bars, language providers, and event listeners
...
7. **Event Listeners** - Always unsubscribe from events in deactivate
8. **UI Elements** - Dispose all UI elements (status bars, tree views, output channels)
9. **Test Lifecycle** - Write tests for activation/deactivation with all resources
