...
export default definePlugin({
  async activate(context) {
    // Access the API directly from context
    ...
    context.subscriptions.push(
      api.commands.register({
        id: 'myPlugin.hello',
        title: 'Say Hello',
        handler: () => api.ui.showNotification('Hello!', 'info')
      })
    )
  }
})
...
Plugins run in a secure sandbox with access to specific globals and resources.

### Globals

*   **Standard JS**: `console`, `setTimeout`, `setInterval`, `Promise`, `JSON`, etc.
*   **Lokus SDK**: `window.LokusSDK` (automatically aliased in imports).
*   **React**: `window.React` and `window.ReactDOM` are available globally.

### CSS Loading

The runtime automatically loads and injects styles from the following files if they exist in your plugin root:
*   `style.css`
*   `styles.css`
*   `index.css`
...
To load static assets (images, icons) from your plugin directory, use the `assetUri` provided in the context:

```typescript
const imageUrl = `${context.assetUri}/assets/icon.png`;
```
