...
3. **Registry Account**
   ...
   - **Access Token** generated from [Dashboard > Settings > Tokens](https://registry.lokus.dev/dashboard/settings/tokens)
...
1. Go to **[Dashboard](https://registry.lokus.dev/dashboard)**
2. Navigate to **Settings > Tokens**
3. Click "Generate New Token"
4. Name: "CLI Publishing"
5. Permissions: Select `publish:plugin`
6. Copy the token (starts with `lpt_...`)

> **Note:** Tokens are only shown once. Store them securely.

### 3. Login via CLI

You can login interactively or via flags:
...
```bash
lokus-plugin login --token lpt_YOUR_TOKEN
```

This saves your credentials to `~/.lokus/config.json`.

Alternatively, use an environment variable (good for CI/CD):

```bash
export LOKUS_REGISTRY_TOKEN=lpt_YOUR_TOKEN
```
