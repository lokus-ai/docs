...
- `--ui` - Open test UI (Vitest only)
- `--reporter <name>` - Custom reporter
- `--updateSnapshot` - Update snapshots
- `--detectOpenHandles` - Detect open handles (Jest)
...
```bash
# Run all tests (auto-detects framework)
lokus-plugin test
...
# Specific tests
lokus-plugin test --pattern "user-service"
```
...
- `-f, --format <format>` - Format: `zip`, `tar`, or `both`
- `-t, --target <target>` - Target: `production` or `development`
...
# Development build with source maps
lokus-plugin package --target development --sourcemap

# Multi-format output
lokus-plugin package --format both

# Include docs and source
lokus-plugin package --include-docs --include-source
```
...
Generate comprehensive documentation for your plugin.
...
**Options:**
- `-g, --generator <type>` - Generator type: `typedoc`, `jsdoc`, `custom`, or `auto` (default)
- `-o, --output <dir>` - Output directory (default: `docs`)
- `-s, --serve` - Start local server after generation
- `-p, --port <number>` - Server port (default: 8080)
- `-w, --watch` - Watch mode
- `--include-source` - Include source code in docs
- `--include-examples` - Include examples directory
- `--theme <theme>` - Documentation theme

**Examples:**

```bash
# Generate docs (auto-detect)
lokus-plugin docs

# Serve locally
lokus-plugin docs --serve

# Use TypeDoc specifically
lokus-plugin docs --generator typedoc --theme dark
```
