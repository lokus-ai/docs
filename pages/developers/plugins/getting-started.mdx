...
npx lokus-plugin create my-first-plugin

# Or use quick mode with defaults
npx lokus-plugin create my-first-plugin --skip-prompts --template basic-typescript
...
When your plugin is instantiated, it receives a `PluginContext` with these APIs:
...
If your plugin needs to interact with the editor:
...
| Method | Description |
|--------|-------------|
| `addSlashCommand()` | Add a slash command to the editor |
| `addToolbarItem()` | Add a button to the editor toolbar |
| `addContextMenuItem()` | Add item to right-click menu |
| `addKeyboardShortcut()` | Register a keyboard shortcut |
| `getSelection()` | Get current text selection |
| `replaceSelection()` | Replace selected text |
| `getText()` | Get editor content as text |
| `insertNode()` | Insert a node at cursor position |
...
- [Plugin API Reference](/developers/plugins/api) - Full API documentation
...
- [UI Plugins](/developers/plugins/ui-plugins) - Create custom UI panels
